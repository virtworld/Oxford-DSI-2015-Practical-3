...
void CreateR(int numOfRecR, int numOfRecS)
{
	Status s;
	...
	Employee e;
	RecordID rid;
	short permutation[numOfRecR];

	RandomPermutate (permutation, numOfRecR); // generate a random array of integer

	for (int i = 0; i < numOfRecR; i++)
	{
		e.id   = permutation[i];
		e.age  = rand() % 20 + 20;
		e.proj = rand() % numOfRecS;
...
void CreateS(int numOfRecR, int numOfRecS)
{
	Status s;
	...
	Project e;
	RecordID rid;
	short permutation[numOfRecS];

	RandomPermutate (permutation, numOfRecS);

	for (int i = 0; i < numOfRecS; i++)
	{
		e.id   = permutation[i];
		e.manager  = rand() % numOfRecR;
		e.fund = (rand() % 500)*10;
		e.status = rand() % 5;
